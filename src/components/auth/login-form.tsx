...
export function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.result) throw new Error('Không nhận được token từ server');
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Button type="submit" disabled={loading} className="w-full h-11 rounded-lg font-semibold tracking-wide">
        {loading ? (
          <>
