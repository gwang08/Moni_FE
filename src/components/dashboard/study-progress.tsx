...
// Format date cell for native SVG <title> tooltip (browser-rendered, zero jitter)
function formatCellTitle(dateStr: string | undefined, count: number): string {
  if (!dateStr) return '';
  const d = new Date(dateStr);
  const dmy = `${String(d.getDate()).padStart(2, '0')}/${String(d.getMonth() + 1).padStart(2, '0')}/${d.getFullYear()}`;
  return count === 0 ? `${dmy} — Không có hoạt động` : `${dmy} — ${count} bài luyện tập`;
}
...
export function StudyProgress() {
  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm h-full">
      {/* Heatmap styling — no transform on hover to avoid jitter that breaks tooltip */}
      ...
      <div className="overflow-x-auto pb-1">
        <div className="min-w-[720px]">
          <CalendarHeatmap
            ...
            titleForValue={(value: any) => formatCellTitle(value?.date, value?.count ?? 0)}
          />
        </div>
      </div>
