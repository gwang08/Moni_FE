...
import { bandTextColor, bandBarGradient } from './color-helpers';
...
interface Props {
  ...
  skillChipLabel: string; // VD: "Writing · Task 2", "Speaking · Phiên #6"
  title: string;
  metaItems?: MetaItem[];
}

// Hero chung cho cả Writing và Speaking — gradient teal + overall band ring + 4 criterion bars
export function ResultHero({ overall, criteria, skillChipLabel, title, metaItems }: Props) {
  const ringPct = Math.min(100, (overall / 9) * 100);
    ...
    <div className="rounded-3xl bg-white border border-slate-100 overflow-hidden shadow-[0_1px_2px_rgba(15,23,42,0.04),0_8px_28px_-12px_rgba(15,23,42,0.08)]">
      <div className="relative p-7 md:p-8 bg-gradient-to-br from-teal-600 via-teal-500 to-emerald-400 text-white">
        ...
        <div className="relative flex flex-col md:flex-row items-start md:items-center gap-6 md:gap-10">
          {/* Overall band ring */}
          <div
            className="relative w-32 h-32 rounded-full shrink-0 shadow-[0_10px_30px_-8px_rgba(0,0,0,0.35)] grid place-items-center"
            style={{ background: `conic-gradient(#14B8A6 0 ${ringPct}%, rgba(255,255,255,0.2) 0)` }}
          >
            <div className="w-[108px] h-[108px] rounded-full bg-white grid place-items-center">
              <div className="text-center leading-none">
                <div className="text-[40px] font-black text-teal-700 tracking-tight">{overall.toFixed(1)}</div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-[20px] md:text-[24px] font-black leading-tight tracking-tight">{title}</h1>
        ...
        {criteria.map((c) => (
          <div key={c.key} className="p-5">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-[18px] font-black ${bandTextColor(c.band)} tabular-nums`}>
                {c.band.toFixed(1)}
              </span>
            </div>
