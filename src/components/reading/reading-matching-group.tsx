...
export function ReadingMatchingGroup({ questions, answers, submitted, onAnswer }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {selectedPillId && !submitted && (
        <p className="text-xs text-blue-600 animate-pulse">Chọn 1 câu hỏi bên dưới để gán heading</p>
      )}
              ...
              <div className="mt-2 ml-7">
                {assignedOpt ? (
                  <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm ${
                    ...
                    {!submitted && (
                      <button type="button" onClick={() => handleClearSlot(question.id)} className="text-blue-400 hover:text-blue-600">
              ...
              {submitted && correctOpt && assignedId != null && (
                <div className="mt-2 ml-7 flex items-center gap-1.5 text-xs">
                  {isCorrect ? (
                    ...
                    <><XCircle className="h-3.5 w-3.5 text-red-500" /><span className="text-red-600">Đáp án đúng: <strong>{correctOpt.content}</strong></span></>
                  )}
                </div>
              )}

              {submitted && question.explanation?.text && (
                <div className="mt-2 ml-7 pt-2 border-t border-gray-100">
