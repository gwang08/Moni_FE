  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm h-full">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .react-calendar-heatmap rect:hover { stroke: #f97316; stroke-width: 2px; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); transform: scale(1.1); transform-origin: center; }
        .react-calendar-heatmap rect { transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); }
      `}} />

      <TooltipProvider>
        {/* Header — title + inline stat chips */}
              ...
              transformDayElement={(element, value: any) => {
                ...
                return (
                  <Tooltip key={value?.date || Math.random()}>
                    <TooltipTrigger asChild><g>{element as any}</g></TooltipTrigger>
                    <TooltipContent className="bg-gray-900 text-white font-bold px-3 py-2 rounded-xl shadow-xl border-none">
